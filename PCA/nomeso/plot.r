setwd("/media/maxlaubstein/data1/STJARangewideGenomics/PCA2/nomeso")
library(ggplot2)
library(readr)
library(readxl)
library(ggrepel)
#library(ggfun), for element_roundrect

pca <- read_table("/media/maxlaubstein/data1/STJARangewideGenomics/PCA2/nomeso/Cyanocitta_PCA_nomeso.eigenvec", col_names = FALSE)
eigenval <- scan("/media/maxlaubstein/data1/STJARangewideGenomics/PCA2/nomeso/Cyanocitta_PCA_nomeso.eigenval")
...
pca$Group <- factor(pca$Group, levels = c("Coastal (Pops. 1-68)", "Interior Northwest (Pops. 69-79)", "Teton (Pops. 80-82)", "N. Contact Zone (Pops. 83-85)", "S. Contact Zone (Pops. 86-88)", "Rocky Mountains (Pops. 89-102)"))

scz <- subset(pca, pca$Group == "S. Contact Zone (Pops. 86-88)")

library(ggstar)

plot <- ggplot(pca, aes(X3, X4, col = Group, label = popnumber, shape = Group))+
  geom_point(size = 3)+
  theme_minimal()+
  scale_colour_manual(values = alpha(c("#7eb0d5","#b2e061", "#FFD23F" , "#FFD23F", NA, "#fd7f6f"), 0.5))+
  scale_shape_manual(values = c(19, 19, 17, 18, 4, 19))+
  geom_star(data = scz, aes(X3, X4), fill = "#FFD23F", color = NA, starshape = 29, alpha = 0.5, size = 3.5)+
...
ggsave(plot = plot + theme(legend.position = "none"), filename = "/media/maxlaubstein/data1/STJARangewideGenomics/plots/PCA_nomeso.pdf", device = "pdf", height = 3.5, width = 3.5, units = "in")
