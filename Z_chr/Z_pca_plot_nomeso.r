...
pca <- read_table("Z_PCA_nomeso.eigenvec", col_names = FALSE)
eigenval <- scan("Z_PCA_nomeso.eigenval")
...
pca$Group <- factor(pca$Group, levels = c("Coastal (Pops. 1-68)", "Interior Northwest (Pops. 69-79)", "Teton (Pops. 80-82)", "N. Contact Zone (Pops. 83-85)", "S. Contact Zone (Pops. 86-88)", "Rocky Mountains>
...
ggsave(plot = plot + theme(legend.position = "none"), filename = "Z_PCA_nomeso.pdf", device = "pdf", height = 3.5, width = 3.5, units = "in")
