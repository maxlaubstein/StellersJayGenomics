...
samplename<-read.table("../admixture/plink.fam")$V2
...
files <- c("../admixture/plink_STJA_forAdmix.2.Q",
           "../admixture/plink_STJA_forAdmix.3.Q",
           "../admixture/plink_STJA_forAdmix.4.Q",
           "../admixture/plink_STJA_forAdmix.5.Q",
           "../admixture/plink_STJA_forAdmix.6.Q",
           "../admixture/plink_STJA_forAdmix.7.Q",
           "../admixture/plink_STJA_forAdmix.8.Q",
           "../admixture/plink_STJA_forAdmix.9.Q")
...
cat_admx$color <- NA
for(i in (unique(cat_admx$k))) {
  dat <- cat_admx[cat_admx$k == unique(cat_admx$k)[1],]
  for(j in 1:length(unique(dat$popGroup))){
    if(dat[dat$sampleID=="MVZ181877",]$proportion[j] > 0.9) {dat$color[j] <- 'blue'}
  }
}

foo <- cat_admx[cat_admx$sampleID=="MVZCCGP-Cst25_II-E01",]
foo$popGroup[which(foo$proportion > 0.9)]



#which popgroup has prop > 0.9 in MVZCCGP-Cst29_II-G01 we designate as 'blue'
